...
# from nets.frcnn_training import  AnchorTargetCreator, ProposalTargetCreator
'''
...
if __name__ == "__main__":
    ...
    if True:
        ...
        # torch.save(model, 'model_data/faster_rcnn_random_init.pth')
        #---------------------------------------#
        #   开始模型训练
        #---------------------------------------#
        total_loss, val_loss, val_loc_loss, val_cls_loss, val_roi_loc_loss, val_roi_cls_loss=[],[],[],[],[],[]


        #
        checkpoint = torch.load('model_data/coco_init_checkpoint.pth.tar')
        model.load_state_dict(checkpoint['state_dict'])  # 模型参数
        optimizer.load_state_dict(checkpoint['optimizer'])  # 优化参数
        Init_Epoch = checkpoint['epoch']
        df = pd.read_csv('df_losses_init_coco.csv')
        total_loss = df['total_loss'].tolist()
        val_loss = df['val_loss'].tolist()
        val_loc_loss = df['val_loc_loss'].tolist()
        val_cls_loss = df['val_cls_loss'].tolist()
        val_roi_loc_loss = df['val_roi_loc_loss'].tolist()
        val_roi_cls_loss = df['val_roi_cls_loss'].tolist()
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            a,b,c,d,e,f = fit_one_epoch(model, train_util, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir)
            total_loss.append(a)
            val_loss.append(b)
            val_loc_loss.append(c)
            val_cls_loss.append(d)
            val_roi_loc_loss.append(e)
            val_roi_cls_loss.append(f)
            ...
            torch.save(state, 'model_data/coco_init_checkpoint.pth.tar')
            torch.save(model, 'model_data/coco_init_fasterrcnn.pth')

            df = pd.DataFrame({'total_loss': total_loss , 'val_loss':val_loss ,
                               'val_loc_loss': val_loc_loss , 'val_cls_loss':val_cls_loss ,
                               'val_roi_loc_loss': val_roi_loc_loss , 'val_roi_cls_loss':val_roi_cls_loss})
            df.to_csv('df_losses_init_coco.csv', index=False)
