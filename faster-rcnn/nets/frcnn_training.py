...
class FasterRCNNTrainer(nn.Module):
    ...
    '''
    imgs, bboxes = images, boxes
    scale=1
    base_feature = model_train(imgs, mode = 'extractor')
    rpn_locs, rpn_scores, rois, roi_indices, anchor = model_train(x = [base_feature, img_size], scale = scale, mode = 'rpn')
    a = rois.detach().cpu().numpy()
    np.save(file='候选框.npy',arr=a)
    '''
